...
public class Conge {
    ...
    public Conge(int id_conge, Date leave_start, Date leave_end, int number_of_days, Statut status, int id_employe, Type type, Reason reason, String description) {
    ...
    public Conge(Date leave_start, Date leave_end, int number_of_days, Statut status, int id_employe, Type type, Reason reason, String description) {
        this.leave_start = leave_start;
        this.leave_end = leave_end;
        this.number_of_days = number_of_days;
        this.status = (status == null) ? Statut.EN_COURS : status;
    ...
    public boolean equals(Object o) {
        ...
        Conge conge = (Conge) o;
        return id_Conge == conge.id_Conge &&
                number_of_days == conge.number_of_days &&
                id_employe == conge.id_employe &&
                num_tt_restant == conge.num_tt_restant &&
                num_conge_restant == conge.num_conge_restant &&
                Objects.equals(leave_start, conge.leave_start) &&
                Objects.equals(leave_end, conge.leave_end) &&
                status == conge.status &&
                type == conge.type &&
                reason == conge.reason &&
                Objects.equals(description, conge.description);
    }

    @Override
    public int hashCode() {
        return Objects.hash(id_Conge, leave_start, leave_end, number_of_days, status, id_employe, type, reason, description, num_tt_restant, num_conge_restant);
    }
